...
function OrderPage() {
  ...
  const {
    ...
    run: runAllOrder,
    loading,
  } = useRequest(() => serviceGetAllOrder(user?.account.getUser.id, status), { manual: true });
                        ...
                        return (
                          <div key={item.ID} className={clsx(styles.order_product)}>
                            ...
                            <div className={clsx(styles.order_product__right)}>
                              <span
                                className={clsx(
                                  styles.order_product__right__discount,
                                  'text-[16px]',
                                )}
                              >
                              ...
                              >
                                {FormatPrice.format(
                                  item.giaBan - (item.giaBan * item.giamGia) / 100,
                                )}
                                đ
                              </span>
                            </div>
                    ...
                    <div className={clsx(styles.order_bottom)}>
                      ...
                      <div className={clsx(styles.order_purchase)}>
                        {order[0].Order.trangThaiDH === 0 ? (
                          ...
                          <button className={clsx(styles.order_cancel__btn)}>
                            Đơn hàng đã hủy
                          </button>
                        )}
